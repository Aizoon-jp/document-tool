...
前提:
  ...
  - 取引先名は MOCK_CLIENTS（c1/c2/c3）で名寄せされる（株式会社サンプル / 有限会社テスト商事 / 合同会社ダミー）
...
| E2E-DOC-HIST-006 | 複合条件＋リセット | 取引先名 `ダミー` + 種別 `請求書` を適用 → `リセット` ボタンをクリック | 絞り込み中は合致行のみ表示、リセット後は15件に戻り全入力欄が初期値に戻る |
...
- **期待結果**:
  - 手順1〜2後: `tbody > tr` が **3行**（d006 / d009 / d012: すべて `合同会社ダミー` × `invoice`）、件数表示 `15件中 3件を表示`
