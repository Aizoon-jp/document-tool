...
### 現時点: Anthropic Claude API（名刺OCRのみ）
- **用途**: 取引先マスタの「名刺から登録」。名刺画像から会社名・住所・電話・担当者等を抽出する
- **モデル**: `claude-sonnet-5`（高解像度画像対応。旧字体の誤変換を避けるため Haiku 4.5 から変更）
- **通信**: メインプロセスからのみ発火。レンダラーのCSPは `connect-src 'self'` のまま
- **APIキー**: `electron-store`（app-settings）にローカル保存。設定画面の「データ」タブで登録・削除
- **未設定でも他の全機能は動作する**。名刺読み取りを使わない限りネット接続は不要
...
### 判定結果: 名刺OCRのみAI利用（単発呼び出し）
書類作成の本体は純粋なCRUD + PDF生成。AIは名刺→取引先マスタの入力補助に限定する。

- 構成: マルチモーダルLLMへの**単発呼び出し**（エージェント型でもパイプライン型でもない）
- 出力: JSON Schema による構造化出力（`output_config.format`）で項目を固定
- **ハルシネーション対策**: 読み取り結果は自動保存せず、必ず確認・修正ダイアログを挟んでから保存する
- 既存カラムに無い項目（役職/メール/携帯/FAX/URL）は備考欄にまとめ、DBスキーマは変更しない
